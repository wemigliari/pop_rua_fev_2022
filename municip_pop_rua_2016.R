...
names(brasil_munic_2016)[1]<- "codigo_ibg"

munic_2016_pop_rua  <- data.frame(count(brasil_munic_2016, "codigo_ibg"))
...
brasil_munic_2016_pop_rua <- data <- merge(munic_2016_pop_rua, brasil, by = "codigo_ibg")
...
write_xlsx(brasil_munic_2016_pop_rua, "/Users/wemigliari/Documents/R/tabelas/min_cid_observatorio/municipios_pop_rua_2016.xlsx")
...
mybins <- c(0, 10, 20, 30, 50, 100, 200, 500, 600, 800, 1000, 2000, 3000, 6500, 25100)
...
mytext <- paste(
  "Município: ", brasil_munic_2016_pop_rua$nome, "<br/>",
...
leaflet(shp_joined_2016) %>% 
  ...
  addLegend(pal=mypalette, values=~brasil_munic_2016_pop_rua$freq, opacity=1, title = "Municípios, Totais de Moradores em Situação de Rua, 2016", position = "bottomright" )%>%
