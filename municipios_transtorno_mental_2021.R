...
brasil_munic_2021_pop_rua <- merge(brasil_munic_2021, brasil, by = "codigo_ibge")
...
sum(brasil_munic_2021_pop_rua$IN_DEF_TRANSTORNO_MENTAL_MEMB)
...
sum(brasil_munic_2021_pop_rua$freq)
...
shp_joined_2021 <- st_as_sf(brasil_munic_2021_pop_rua) 
...
mypalette <- colorBin( palette=mycolors, domain=brasil_munic_2021_pop_rua$freq, na.color="transparent", bins=mybins)

mytext <- paste(
  "Município: ", brasil_munic_2021_pop_rua$NM_MUN, "<br/>",
  "Total: ", brasil_munic_2021_pop_rua$freq, "<br/>",
  sep="") %>%
...
leaflet(shp_joined_2021) %>% 
  ...
  addLegend(pal=mypalette, values=~brasil_munic_2021_pop_rua$freq, opacity=1, title = "Municípios, Transtorno Psicológico Moradores em Situação de Rua, 2021", position = "bottomright" )%>%
